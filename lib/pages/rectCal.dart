...
class _inputBoxState extends State<inputBox> {
  ...
  final _smuy1 = TextEditingController();
  ...
  bool _switch = false, _isshort = false, _refreshed = false, _isUni = false, _isBi = false;
                                            ...
                                            ? Column(
                                                children: [
                                                  Container(
                                                      child: (_mux.text == 0)
                                                          ? const alertB(
                                                              disc: 'momunt cant be zero',
                                                            )
                                                          : null),
                                                  resultStyle("muxmuy = $_muxmuy \n mut = $_mut", Alignment.center, 1),
                                                  resultStyle("Seclect fy = $_selectedfy and d'/D =$efcd graph ", Alignment.center, 1),
                                                  Container(child: (_isBi) ? resultStyle("d'/b = $_efcb", Alignment.center, 0.6) : null),
                                                  ...
                                                  resultStyle(' Gps value = $_gps', Alignment.center, 0.6),
                                                ],
                                              )
                                            : null,
                                      ),
  ...
  void onPressed() {
    ...
    if (_switch == false) {
      setState(() {
        ...
        double temp1;
        temp1 = ag / double.parse(_b.text);
        int temp2 = temp1.ceil();
        while (temp2 % 10 != 0) {
          temp2 = temp2 + 1;
        }

        _D.text = temp2.toString();
      });
    } else {
      setState(() {
    ...
    if (_check < 12.0 || _check1 < 12.0) {
      setState(() {
        ...
        if ((ex <= exmin) && (ey <= eymin)) {
          ...
          setState(() {
            _isUni = true;
            esc = 'Column is Uni-axially loaded in X direction';
            ...
            _x = double.parse(_mux.text) / (double.parse(_selectedfck.toString()) * b * d * d);
          ...
          setState(() {
            _isUni = true;
            esc = 'Column is Uni-axially loaded in Y Direction';
            efcd = double.parse(_efc.text) / double.parse(_D.text);
            if (efcd <= 0.05) {
              efcd = 0.05;
            } else if (efcd > 0.05 && efcd <= 0.1) {
          ...
          setState(() {
            ...
            _efcb = double.parse(_efc.text) / double.parse(_b.text);
            if (_efcb <= 0.05) {
              _efcb = 0.05;
            } else if (_efcb > 0.05 && _efcb <= 0.1) {
              _efcb = 0.1;
            } else if (_efcb > 0.1 && _efcb <= 0.15) {
              _efcb = 0.15;
            } else {
              _efcb = 0.2;
            }
            double tempx = double.parse(_mux.text);
            double tempy = double.parse(_muy.text);
            _muxmuy = (tempx * tempx) + (tempy * tempy);
            double _muxmuy1 = sqrt(_muxmuy);
            _mut = 1.15 * _muxmuy1;
            double b = double.parse(_b.text), d = double.parse(_D.text);
            _x = (_mut * 1000000) / (double.parse(_selectedfck.toString()) * b * (d * d));
            _y = _pu / (double.parse(_selectedfck.toString()) * b * d);
            _gps = double.parse(_gpsfck.text) * double.parse(_selectedfck.toString());
            _as = (_gps / 100) * (double.parse(_b.text) * double.parse(_D.text));
          });
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          ...
          TextFormField(
            validator: (str) {
              (str == null) ? 'enter the valid input' : null;
            },
            keyboardType: const TextInputType.numberWithOptions(decimal: true, signed: false),
            inputFormatters: [
              FilteringTextInputFormatter.allow(RegExp(r'^\d+\.?\d{0,2}')),
          ...
          ElevatedButton(
              onPressed: () {
                ...
                setState(() {
                  ...
                  scb = (double.parse(_b.text) - (2 * double.parse(_efc.text)));
                          ...
                          onPressed: () {
                            setState(() {
                              ...
                              if (_isUni) {
                                ...
                                _muy1 = double.parse(_smuy1.text) * double.parse(_selectedfck.toString()) * double.parse(_b.text) * double.parse(_D.text) * double.parse(_D.text);
                                _muy1 = _muy1 / 1000000;
                              }
